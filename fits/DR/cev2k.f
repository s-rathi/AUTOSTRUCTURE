...
C
C     CONVERT A cfout FILE FROM EV TO K.
C
      PARAMETER (NUMT=20)
C
      CHARACTER*1 CARD(60)
...
C
   5  READ(8,101,END=20)CARD
      IF(CARD(1).EQ.'Z')THEN             !CHECK FOR NEW HEADER
        WRITE(9,101)CARD
        GO TO 1
      ELSE
        BACKSPACE(8)
      ENDIF        
      READ(8,*)TEV,ADR,ADRF0,DIFF0
C
      TK=TEV/8.6174E-5
C
      CALL FUNCS(TK,A,ADRF,DUM,NFIT)
C
      ADRF=ADRF/(TK*SQRT(TK))
C
      DIFF=0.0
      IF(ADR.GT.0.0)DIFF=ABS(ADR-ADRF)/ADR
      WRITE(9,200)TK,ADR,ADRF,DIFF
      GO TO 5
C
  20  STOP 'OUTPUT IN cfoutk'
C
