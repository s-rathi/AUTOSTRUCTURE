...
C     0, 1 OR 2 NON-ZERO TEMP INDEXES TO FIX ADDITIONAL PAIRS AT
...
C
  15  WRITE(*,*)'USE ONE OR TWO FIXED PAIRS AT (LOW) T INDEX?',
     X' [e.g. 1 5, or 0 0 for none]:'
      READ(*,*)I0,I1
      IF(I0.GT.0)THEN
...
C     WRITE(8,201)
C     DO N=1,NTEMP
C       CALL FUNCS(TEMP(N),A,ADRF(N),DUM,NFIT)
C       T=0.0
C       IF(ADR(N).GT.0.0)T=ABS(ADR(N)-ADRF(N))/ADR(N)
C       T3=TEMP(N)*SQRT(TEMP(N))
C       WRITE(8,200)TEMP(N),ADR(N)/T3,ADRF(N)/T3,T
C     ENDDO
C
C
      GO TO 10
