...
C
c  10  WRITE(*,*)
c      WRITE(*,*)'ENTER No. OF SETS OF FREE FITTING PARAMETERS',
c     X' [.le. 0 to exit]:'
C       READ(*,*)NFIT         !***MINIMUM UNCOMMENT FOR INTERACTIVE USE
c
C
c      IF(NFIT.LE.0)STOP 'OUTPUT IN FILE cfout'
C
      NFIT=4*NFIT
...
c      DO I=1,NFIT,4
c        WRITE(99,203)I,A(I),A(I+1),A(I+2),A(I+3)
c      ENDDO
c      CALL FLUSH(99)
...
C
      ...
      DO I=1,NFIT,4
        II=(I+3)/4
        WRITE(8,203)II,A(I),A(I+1),A(I+2),A(I+3)
      ENDDO
      WRITE(8,201)
      DO N=1,NTEMPR
...
C
 ...
 202  FORMAT('  N','  FITTING COEFFICIENTS: A, B, T0, T1')
 203  FORMAT(I3,1PE11.3,0PF8.4,1P,2E11.3)
...
C
      ...
      DO I=1,NA,4
        T0=SQRT(X/A(I+2))
        T1=SQRT(X/A(I+3))
        Y=Y+LOG(A(I))-LOG(T0)-(1.D0-A(I+1))*LOG(1.D0+T0)
     X                       -(1.D0+A(I+1))*LOG(1.D0+T1)
        DYDA(I)=1.D0/A(I)
