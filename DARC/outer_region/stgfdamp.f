...
C
C                     PROGRAM STGFDAMP
C                   BASED-ON STGF v4.9
C
C  RADIATION DAMPED CALCULATIONS FOR FREE ELECTRONS.
C
...
C         33 RESERVED
C         34 RESERVED
C         36 FOR UNPHYSICAL K-MATRIX. FILE kmatls.
C         37 RESERVED
C         38 RESERVED
C         39 RESERVED
C
C
C     OPEN(5,FILE='dstgfdamp',STATUS='UNKNOWN')
      OPEN(6,FILE='routfdamp',STATUS='UNKNOWN')
C
...
C
     ...
     X             MZSLP,MZTET,MZDEG,MZNRG,MZMET,MZDEC,MZREC,MZMEG
C
C
...
C     NODAMP  > 0  SWITCH OFF ALL DAMPING (OVERRIDES ANY OTHER NTYP SETTING).
C             = 0  USE GIVEN/DEFAULT DAMPING NTYP SWITCHES (DEFAULT).
C     IAUGER  = 0  (DEFAULT)
C             = 1  READ CORE RATES FROM FILE AUGER: AAUNITS, THEN ITARG,
C                  AA(AAUNITS) WHERE Z-SCALED AA(RYD)=AA(AAUNITS)/AAUNITS
C                  I.E. AAUNITS/Z^2= 1 RYD, 0.5 AU, 2.06706E16 /SEC (1/HBAR).
...
C           = 2 WRITE UNPHYSICAL K-MATRIX DATA TO kmatls. (TOM)
...
C
C DAMPING SPECIFIC VARIABLES (DEFAULTS: EXCITATION ON, DR/RR OFF)
C
      NTYP1=1
      NTYP2I=1
...
C
      IF(NODAMP.GT.0)THEN       !SWITCH OFF DAMPING - EG NEUTRALS
        NTYP1=0
        NTYP2I=0
        NTYP2OR=0
        NTYP2OF=0
...
C
      ...
      IF(IQDT.LT.0)THEN                    !NTYP1 ONLY WHEN SQDT
        IF(NTYP2I+NTYP2OF+NTYP2OR.GT.0)WRITE(6,*)' ***WARNING: NTYP2'
     X, ' RADIATION SWITCHED-OFF'
        NTYP2I=0
        NTYP2OF=0
        NTYP2OR=0
      ENDIF
...
C
      ...
      ENDIF
C
      IF(IPRKM.NE.0.AND.IMODE.LT.0)THEN
        WRITE(6,*)' *** ERROR: SET IPRKM=0 FOR INTERPOLATION, IMODE=-1'
        STOP ' *** ERROR: SET IPRKM=0 FOR INTERPOLATION, IMODE=-1'
      ENDIF
...
C
      ...
      IF(QETEST.LE.ZERO)QETEST=1.E-10
...
C
 3999 WRITE(6,601)IPRINT,IRAD,IPERT,LMX,AC,IMESH,IOPT1,IRDEC,IMODE,IQDT
     ...
     X       ,NTYP2OF,NTYP2OR,NMIN,IGAUGE,IAUGER
C
C
...
C AAUNITS/Z^2 = 1 RYD, 0.5 AU, 2.06706E16 /SEC I.E. 1/HBAR
        DO I=1,MZTAR
          AAUGER(I)=TZERO
        ENDDO
        READ(4,*)AAUNITS
...
C  ***********************************
C
      IF(NTYP2I.NE.0)CALL BDORG
C
C WRITE UNPHYSICAL K-MATRIX DATA
C
      IF(IPRKM.EQ.2.AND..NOT.PQRD)THEN
        NAMKLS='kmatls'//NUM(KASE)
        OPEN(36,FILE=NAMKLS,FORM='UNFORMATTED')
        WRITE(36)NSPN2,LRGL2,NPTY2
        WRITE(36)NAST
        DO I=1,NAST
          WRITE(36)I,ISAT(I),LAT(I),ENAT(I)
        ENDDO
        WRITE(36)NCHF
        DO I=1,NCHF
          WRITE(36)I,ITARG(I),LLCH(I),KJ(I),ECH(I)
        ENDDO
        WRITE(36)MXE
      ENDIF
...
C
 ...
 6000 FORMAT(///1X,70('+')//10X,'STGFDAMP UoS v4.8'/
     + 10X,17('*')//5X,'HISTORY OF MODIFICATIONS'//
 ...
 6005 FORMAT(9X,'(20) INFINITE ENERGY OMEGA ADDED (DIPOLE ONLY)'
     ...
     + /9X,'(26) NEUTRAL CASE ADDED'
     + /9X,'(27) DARC HD.DAT (AND H.DAT VIA DARC INTERFACE)'
     + /9X,'(28) PARTITIONED R-MATRIX'
     +)
 6001 FORMAT(//
 ...
 6002 FORMAT(//10X,'COMPILED FOR DIMENSIONS -'//
     ...
     + 15X,'METASTABLES FOR DR         MZMET =',I6/
     + 15X,'RAD DECAY STATES TYPE-II   MZDEC =',I6/
...
C
          ...
          ENDIF
C
          CALL SC(EPS(I),LLCH(I),RZERO,AC,S(I),SP(I),C(I),CP(I),IERR)
C
...
c            write(*,*)i,llch(i),h,h0,wh,kstart
            ...
            IFLEG=IFLEG0
C
            CALL SC(EPS(I),LLCH(I),RSTART,AC,S(I),SP(I),C(I),CP(I),IERR)
C
...
C
            ...
            IF(IOMIT(I).LE.0.AND.RINF(I).GT.TZERO)THEN
              ...
              IF(KINF.GT.1)THEN
                ...
                DO K=1,KINF
                  FS(K,I)=FST(K)
                ENDDO
                CALL SC(EPS(I),LLCH(I),RINF0,AC,S(I),SP(I),C(I),CP(I)
     X                 ,IERR)
                CP0=CP(I)
     ...
     X                      ,CP(I),I)
                ...
                IF(ABS(W0-W2).GT.100.*AC)RINF(I)=-RINF(I)
              ENDIF
            ENDIF
            S(I)=FS(1,I)
            SP(I)=FSP(I)
...
C
C  IRDEC   RADIATIVE DECAYS INCLUDED FOR IRDEC=1
C
      WRITE(6,600)
...
C
 3000 IF(ABS(IQDT).EQ.2)THEN
  ...
  999       FORMAT(/72('*')/3X,'WROTE DATA TO ZKMTLS.DAT FOR NSPN2 = ',
     1      I3,3X,'LRGL2 = ',I3,3X,'NPTY2 = ',I3/72('*'))
          ...
          ENDIF
        ENDIF
        IF(IMODE.LT.0.OR.LRGL2.GT.ISKP(IE).OR.NCHOPO.EQ.0
     X     .OR.NOMWRT.EQ.0)GO TO 666
      ENDIF
C
...
C
        ...
        ENDDO
C
C OUTPUT KHI MATRICES FOR JAJOM (KAB,94) AND MQDTS
C
...
C
     ...
     X   LRGL2.GT.ISKP(IE).OR.NCHOPO.EQ.0.OR.NOMWRT.EQ.0)GO TO 666
C
C
C....I/QDT CASE (MQDT ENTRY POINT FOR S-MX)
C
 1000 IF(IQDT.NE.0.AND.TZED.EQ.TZERO)NCHOP=NCHOPO !AS S_PHYS=S_UNPHYS
...
C
          IF(IOPT1.EQ.2)REWIND(21)
   1      READ(21,END=100)IE2,MPTY2,NCHOPP,E2
...
C
      READ(21,END=100)IE2,MPTY2,NCHOPP,E2
...
C
C
      IF(IRAD.GT.0.OR.IPERT.LT.0.OR.IQDT.GT.0)THEN
C
...
CBL   DIMENSION IPIV(MZDEC)
C
      EQUIVALENCE (A2(1,1),C1(1,1))      !STGFDAMP ONLY, NOT STGFBF0DAMP
C
      IF(NTYP2I.EQ.0.OR.NDEC.EQ.0)THEN
        DO J=1,NCHF
          DO I=1,NCHF
