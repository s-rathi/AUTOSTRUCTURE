...
C
CSTRTNBL
      CALL ZLUS(ZA,-NMCJ,NCC,WORK,IERR)
      IF (IERR.NE.0) THEN
        WRITE(6,600)
        STOP 'ERROR IN ZLUS'
      END IF
      CALL ZLUBS(ZA,ZTM,NCC,NCHOP,IERR)
      IF (IERR.NE.0) THEN
        WRITE(6,601)
        STOP 'ERROR IN ZLUBS'
      END IF
...
C
CSTRTBL
CBL   CALL ZSPTRF('L',NCC,ZA,IPIVOT,INFO)
CBL   IF (INFO.NE.0) THEN
CBL      WRITE(6,602) INFO
CBL      STOP 'ERROR IN BLAS ROUTINE ZSPTRF'
CBL   ENDIF
CBL   CALL ZSPTRS('L',NCC,NCHOP,ZA,IPIVOT,ZTM,NCC,INFO)
CBL   IF (INFO.NE.0) THEN
CBL      WRITE(6,603) INFO
CBL      STOP 'ERROR IN BLAS ROUTINE ZSPTRS'
CBL   ENDIF
CENDBL
C
C
...
CB602 FORMAT(//10X,10('*'),' SR. MQDTS: ZSPTRF RETURNED WITH INFO =',I6)
CB603 FORMAT(//10X,10('*'),' SR. MQDTS: ZSPTRS RETURNED WITH INFO =',I6)
      END
C***************************************************************
C
...
C
      ...
      PARAMETER (ONE=1.0)
      PARAMETER (ZERO=(0.0,0.0))
CBL   PARAMETER (ZONE=(1.0,0.0))              !BLAS
      PARAMETER (CONAS=6.476565E-8)           !ALPHA**3/6
      PARAMETER (BIG=170.0)
C
      COMMON/AUGER/AAUGER(NMLT),IAUGER
     ...
     X,FJVJ(NMCJ),ILVJ(NMCJ)
      ...
      COMMON/NRBQDT/QNMAX,FNUMAX
      COMMON/NRBWRK/ZWORK(NMCJ),WORK(NMCJ)
CBL   COMMON/BLWRK/ZTEMP(NMCJ*NMCJ)           !BLAS
C
      PI=ACOS(-ONE)
      IF(ITCC.EQ.0)NCC=NCHPJ(IWJ)-NCHOP
      IF(ITCC.NE.0)NCC=NCHPJX(IWJ)-NCHOP
...
C
CSTRTBL
CBL   CALL ZGEMM('T','N',NCHOP,NCHOP,NCC,ZONE,ZA,NCC,ZTM,NCC
CBL  X           ,ZERO,ZTEMP,NCHOP)
CBL   N=0
CBL   NN=0
CBL   DO ICP=1,NCHOP
CBL     DO IC=ICP,NCHOP
...
CB602 FORMAT(//10X,10('*'),' SR.ZMQDTK: ZSPTRF RETURNED WITH INFO =',I6)
CB603 FORMAT(//10X,10('*'),' SR.ZMQDTK: ZSPTRS RETURNED WITH INFO =',I6)
      END
C******************************************************************
C
...
C     NON-BLAS VERSION. ALSO CONTAINS (COMMENTED-OUT) A BLAS VERSION
C     WHICH USES NO ADDITIONAL MEMORY, UNLIKE THE EXPLICIT BLAS VERSION,
C     BUT IT IS SLOWER.
C
C     T = 2*K/(I+K), S=1-T
C
...
C
CSTRTNBL
      CALL ZVERTS(ZA,-NMCJ,NCHOP,ZWORK,IERR)
      IF (IERR.NE.0) THEN
        WRITE(6,100)
        STOP 'STOP BECAUSE NO INVERSE FOUND IN ZSMTRX'
      END IF
CENDNBL
C
CBL   CALL ZSPTRF('L',NCHOP,ZA,IPIVOT,INFO)
CBL   IF (INFO.NE.0) THEN
CBL      WRITE(6,602) INFO
CBL      STOP 'FAILURE IN BLAS ROUTINE ZSPTRF'
CBL   ENDIF
CBL   CALL ZSPTRI('L',NCHOP,ZA,IPIVOT,ZWORK,INFO)
CBL   IF (INFO.NE.0) THEN
CBL      WRITE(6,603) INFO
CBL      STOP 'FAILURE IN BLAS ROUTINE ZSPTRI'
CBL   ENDIF
C
      CALL ZMULTS(ZTM,ZRJ,ZA,NCHOP)
C
...
C
      RETURN
C
  100   FORMAT(' THE MATRIX: I + K HAS NO INVERSE IN SUBROUTINE '
     1  ,'ZSMTRX - MUST STOP')
CB602 FORMAT(//10X,10('*'),' SR.ZSMTRX: ZSYTRF RETURNED WITH INFO =',I6)
CB603 FORMAT(//10X,10('*'),' SR.ZSMTRX: ZSYTRI RETURNED WITH INFO =',I6)
C
      END
C
C     ******************************************************************
